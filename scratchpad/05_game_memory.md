...
- Enrichment jobs run asynchronously after every recorded engine turn, submitting the turn’s facts plus relevant scene context to a dedicated schema. The response may contain inferred entities, thematic tags, confidence signals, or suggested interpretations of ambiguous text.
- Because enrichment is advisory, results are stored separately from canonical heuristics and annotated with the turn they reference. If an enrichment arrives after additional turns, downstream consumers may ignore it or log it as a late “context hint” rather than altering authoritative fields.
- If enrichment fails or is skipped, the gameplay experience continues using heuristic memory alone; this is the normal path, and heuristics are always the source of truth.
